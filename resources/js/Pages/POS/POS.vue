<template>
  <AuthenticatedLayout>
    <div class="relative p-4 max-h-screen font-poppins">
          ...
          <div v-else>
            ...
            <div class="border rounded-xl p-4">
                  ...
                  <div class="flex justify-between items-start">
                    <div>
                      ...
                      <p class="text-sm text-gray-500 mt-1">
                        <!-- {{ total_sales_per_cashier.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }} -->
                        ₱{{ Number(head.price_snapshot).toFixed(2) }} × {{ head.qty }}
                      </p>
                    </div>

                    <div class="font-semibold text-gray-800">
                      ₱{{ Number(head.subtotal).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="border-t mt-4 pt-4 flex justify-between text-lg font-bold text-gray-900">
                ...
                <span class="text-blue-600">₱{{ baseSubtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <div class="flex justify-between items-start gap-3">
                    ...
                    <div class="text-right space-y-2">
                      ...
                      <span class="text-xs text-gray-400 block">
                        {{ addon.is_void ? 'Voided' : 'Posted' }}
                      </span>

                      <p
                        v-if="addon.is_void && addon.void_remarks"
                        class="text-xs text-red-500 max-w-[180px] break-words"
                      >
                        Remarks: {{ addon.void_remarks }}
                      </p>

                      <button
                        v-if="!addon.is_void"
                        @click="openVoidAddonModal(addon, index)"
                      ...
                      <span
                        v-else
                        class="text-xs px-2 py-1 rounded bg-gray-300 text-gray-700 inline-block"
                      >
                        Voided
                      </span>
                    </div>
            ...
            <div class="border rounded-xl p-4 bg-gray-50">
              ...
              <div class="space-y-2 mb-4">
                <div class="flex justify-between">
                  ...
                  <span>₱{{ baseSubtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                </div>
                ...
                <div v-if="reservationFee > 0" class="flex justify-between text-green-600">
                  <span>Reservation Fee (paid)</span>
                  <span>-₱{{ Number(reservationFee).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                </div>
              ...
              <div class="space-y-3">
                <div
                  ...
                  :key="index"
                  class="rounded-lg border p-3 bg-gray-50 space-y-3"
                >
                  ...
                  <input
                    v-if="lineRequiresReference(line)"
                    type="text"
                ...
                <button
                  ...
                  class="w-full border border-dashed border-gray-400 text-gray-700 px-4 py-2 rounded hover:bg-gray-50"
                >
                  + Add Payment Method
                </button>

                <div class="rounded-lg border bg-white p-3 text-sm space-y-1">
                  <div class="flex justify-between">
                    <span>Total Due</span>
                    <span>₱{{ Number(totalPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Total Paid</span>
                    <span>₱{{ Number(totalPaid).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Remaining</span>
                    <span :class="Number(remainingBalance) > 0 ? 'text-red-600' : 'text-green-600'">
                      ₱{{ Number(remainingBalance).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                    </span>
                  </div>
                  <div v-if="Number(changeAmount) > 0" class="flex justify-between text-green-600 font-medium">
                    <span>Change</span>
                    <span>₱{{ Number(changeAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
                  </div>
                </div>
      ...
      <!-- VOID ADDON MODAL -->
      <div
        v-if="showVoidAddonModal"
        class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
      >
        <div class="bg-white rounded-lg w-full max-w-md p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold">Void Add-on</h2>
            <button @click="closeVoidAddonModal" class="text-gray-500 hover:text-black">
              <i class="fa fa-times"></i>
            </button>
          </div>
          ...
          <div class="space-y-4">
            <div class="bg-gray-50 border rounded p-3">
              <p class="font-medium text-gray-800">
                {{ selectedVoidAddon?.item_name }}
              </p>
              <p class="text-sm text-gray-500 mt-1">
                ₱{{ Number(selectedVoidAddon?.unit_price ?? 0).toFixed(2) }}
                × {{ selectedVoidAddon?.quantity ?? 0 }}
              </p>
            </div>

            <div>
              <label class="block text-sm font-medium mb-1">
                Void Quantity <span class="text-red-600">*</span>
              </label>
              <input
                v-model.number="voidAddonQuantity"
                type="number"
                min="1"
                :max="selectedVoidAddon?.quantity ?? 1"
                class="w-full border rounded px-3 py-2"
              >
              <p class="text-xs text-gray-500 mt-1">
                Max: {{ selectedVoidAddon?.quantity ?? 0 }}
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                v-model="voidAddonRemarks"
                rows="4"
                class="w-full border rounded px-3 py-2"
                placeholder="Enter reason for voiding this add-on"
              ></textarea>
              <p v-if="voidAddonError" class="text-red-600 text-sm mt-1">
                {{ voidAddonError }}
              </p>
            </div>
          </div>
