...
// Toggle sidebar (if controlled by parent)
const toggle = () => {
  emit('toggle')
}
</script>
...
<template>
  <template v-if="role === 5 || role === 8 || role === 9 || role === 10 || role === 11 || role === 12 || role === 13">
    ...
    <div class="space-y-2">
      ...
      >
        <ul v-show="inventoryOpen" class="overflow-hidden space-y-1">
          ...
          <li>
            <Link :href="route('kitchen.inventory.items')" 
          ...
          <li>
            <Link :href="route('kitchen.inventory')" 
              class="pl-8 block px-2 py-1.5 rounded-base hover:bg-gray-200 flex items-center space-x-2">
    ...
    <div class="sm:hidden space-y-1 mt-4">
      ...
      <ResponsiveNavLink :href="route('kitchen.inventory')">Inventory</ResponsiveNavLink>
    </div>
