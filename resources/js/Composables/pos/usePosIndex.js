...
export function usePosIndex() {
  ...
  const payment_methods = ref(page.props.payment_methods ?? [])
  ...
  const showVoidAddonModal = ref(false)
  const selectedVoidAddon = ref(null)
  const selectedVoidAddonIndex = ref(null)
  const voidAddonRemarks = ref('')
  const voidAddonQuantity = ref(1)
  const voidAddonError = ref('')
  ...
  function normalizeHeadCounts(session) {
    return {
      ...
      addons: (session.addons ?? []).map((addon) => ({
        ...
        is_void: Number(addon.is_void ?? 0),
        void_remarks: addon.void_remarks ?? '',
      })),
      reservation_fee: Number(session.reservation?.reservation_fee ?? 0),
    }
  }

  const reservationFee = computed(() => {
    if (cart.value.length === 0) return 0
    return Number(cart.value[0].reservation_fee ?? 0)
  })
  ...
  const mapSessionAddons = (addons = []) => {
    return addons.map((item) => ({
      ...
      is_void: Number(item.is_void ?? 0),
      void_remarks: item.void_remarks ?? '',
    }))
  }
  ...
  const activePaymentMethods = computed(() => {
    return (payment_methods.value ?? []).filter(method => Number(method.is_active) === 1)
  })

  function createPaymentLine() {
    const firstMethod = activePaymentMethods.value[0] ?? null
  ...
  const paymentLines = ref([createPaymentLine()])

  const selectedPaymentMethodById = (id) => {
    return activePaymentMethods.value.find(
      method => Number(method.id) === Number(id)
    ) ?? null
  }

  const addPaymentLine = () => {
    paymentLines.value.push(createPaymentLine())
  }

  const removePaymentLine = (index) => {
    if (paymentLines.value.length <= 1) {
      paymentLines.value = [createPaymentLine()]
      return
    }
  ...
  const getPaymentMethod = (line) => {
    return selectedPaymentMethodById(line.payment_method_id)
  }

  const lineRequiresReference = (line) => {
    return Boolean(getPaymentMethod(line)?.requires_reference)
  }

  const lineIsCash = (line) => {
    return Boolean(getPaymentMethod(line)?.is_cash)
  }
  ...
  const totalCashPaid = computed(() => {
    return paymentLines.value.reduce((sum, line) => {
      if (!lineIsCash(line)) return sum
      return sum + Number(line.amount || 0)
    }, 0)
  })

  const remainingBalance = computed(() => {
    return Math.max(0, Number(totalPrice.value || 0) - Number(totalPaid.value || 0))
  })

  const changeAmount = computed(() => {
    const excess = Number(totalPaid.value || 0) - Number(totalPrice.value || 0)

    if (excess <= 0) return 0

    return Math.min(excess, Number(totalCashPaid.value || 0))
  })
  ...
  const openVoidAddonModal = (addon, index) => {
    if (!addon?.id) {
      showToastMessage('Invalid add-on selected.')
      return
    }

    selectedVoidAddon.value = addon
    selectedVoidAddonIndex.value = index
    voidAddonQuantity.value = 1
    voidAddonRemarks.value = ''
    voidAddonError.value = ''
    showVoidAddonModal.value = true
  }

  const closeVoidAddonModal = () => {
    showVoidAddonModal.value = false
    selectedVoidAddon.value = null
    selectedVoidAddonIndex.value = null
    voidAddonQuantity.value = 1
    voidAddonRemarks.value = ''
    voidAddonError.value = ''
  }

  const voidAddon = async () => {
    if (!selectedVoidAddon.value?.id) {
      voidAddonError.value = 'Invalid add-on selected.'
      return
    }

    const remarks = String(voidAddonRemarks.value || '').trim()
    const qtyToVoid = Number(voidAddonQuantity.value || 0)
    const currentQty = Number(selectedVoidAddon.value.quantity || 0)
    const unitPrice = Number(selectedVoidAddon.value.unit_price || 0)
    const addonId = selectedVoidAddon.value.id
    const addonIndex = selectedVoidAddonIndex.value

    if (!remarks) {
      voidAddonError.value = 'Remarks is required.'
      return
    }

    if (qtyToVoid <= 0) {
      voidAddonError.value = 'Void quantity must be at least 1.'
      return
    }

    if (qtyToVoid > currentQty) {
      voidAddonError.value = 'Void quantity cannot be greater than posted quantity.'
      return
    }
    ...
    try {
      ...
      const response = await axios.post(
        `/${prefix.value}/orders/table-session-addons/${addonId}/void`,
        {
      ...
      if (addonIndex !== null && selectedAddons.value[addonIndex]) {
        const remainingQty = currentQty - qtyToVoid

        selectedAddons.value[addonIndex] = {
          ...selectedAddons.value[addonIndex],
          quantity: remainingQty,
          subtotal: remainingQty * unitPrice,
          is_void: remainingQty <= 0 ? 1 : 0,
          void_remarks: remarks,
          void_qty: (Number(selectedAddons.value[addonIndex].void_qty || 0) + qtyToVoid),
        }
      }

      selectedAddons.value = selectedAddons.value.filter(item => Number(item.quantity) > 0)
      ...
      closeVoidAddonModal()
      showToastMessage(response.data?.message || 'Add-on voided successfully.')
    } catch (error) {
      console.error('Void add-on error:', error)
      voidAddonError.value =
        error.response?.data?.message ||
        error.response?.data?.errors?.void_qty?.[0] ||
  ...
  const addonsTotal = computed(() => {
    return selectedAddons.value.reduce((sum, item) => {
      if (Number(item.is_void) === 1) return sum
  ...
  const totalPrice = computed(() => {
    ...
    return Math.max(0, subtotal.value - discountAmount.value - reservationFee.value)
  })

  const validatePayments = () => {
    if (!paymentLines.value.length) {
      return 'Please add at least one payment method.'
    }
    ...
    for (const line of paymentLines.value) {
      ...
      const method = getPaymentMethod(line)

      if (method?.requires_reference && !String(line.reference_no || '').trim()) {
        return `${method.name} reference no. is required.`
      }
    }

    if (Number(totalPaid.value || 0) < Number(totalPrice.value || 0)) {
      return 'Total payment is insufficient.'
    }
  ...
  const checkoutSelectedTable = async () => {
    ...
    try {
      ...
      const response = await axios.post(`/${prefix.value}/orders/store`, {
        ...
        payments: paymentLines.value.map((line) => ({
          payment_method_id: line.payment_method_id,
          amount: Number(line.amount || 0),
          reference_no: lineRequiresReference(line)
            ? String(line.reference_no || '').trim()
            : null,
        ...
        reservation_fee_used: reservationFee.value,
        reservation_id: cart.value[0]?.reservation?.id ?? null,  // ← add this
      })
  ...
  // }
  const printReceipt = () => {
    const receiptWindow = window.open(`/${prefix.value}/orders/${lastOrderNo.value}/pdf`, '_blank')
